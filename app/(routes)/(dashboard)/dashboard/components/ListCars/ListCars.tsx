...
export  function ListCars({ cars }: ListCarsProps) {
    ...
    const { addLoveItem, lovedItems, removeLoveItem } = useLovedCar()

  return (
    <div className="grid grid-cols-2 gap-6 lg:grid-cols-4">
        {
            cars.map((car: Car) => {
                const {priceDay, photo, id, engine, people, name, cv, transmission, type} = car

                const likedCar = lovedItems.some(item => item.id === id)

                return (
                    <div key={id} className="p-2 rounded-md shadow-md hover:shadow-lg">
